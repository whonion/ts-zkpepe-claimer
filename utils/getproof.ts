...
async function fetchMerkleProof(address: string): Promise<string[] | null> {
  try {
    const response = await axios.get(`https://www.zksyncpepe.com/resources/proofs/${address}.json`, { headers: HEADERS });
...
export async function fetchAndSaveProofs() {
  try {
    ...
    for (const address of addresses) {
      ...
      if (proof) {
        const filePath = `./data/tx_data/proofs/${address}_proof.txt`;
    ...
    console.log('✅ All proofs fetched and saved');
  } catch (error: any) {
...
function getLogger() {
  log4js.configure({
    appenders: { file: { type: 'file', filename: './data/tx_data/proofs.log', layout: { type: 'messagePassThrough' } } },
